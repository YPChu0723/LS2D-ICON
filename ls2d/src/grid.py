...
class Grid_stretched(_Grid):
    def __init__(self, kmax, dz0, nloc1, nbuf1, dz1, nloc2=None, nbuf2=None, dz2=None):
        ...
        if double_stretched:
            ...
            dzdn = (
                dzdn1
                + 0.5 * (dzdn2 - dzdn1) * (1.0 + np.tanh((n - nloc1) / nbuf1))
                + 0.5 * (dzdn3 - dzdn2) * (1.0 + np.tanh((n - nloc2) / nbuf2))
            )
        else:
            dzdn = dzdn1 + 0.5 * (dzdn2 - dzdn1) * (1.0 + np.tanh((n - nloc1) / nbuf1))
...
class Grid_three_stage(_Grid):
    def __init__(self, kmax, dz0, z_stretch_start, stretch_factor, dz_max):
        ...
        while stretch_index < kmax and ((dz0 * (1 + stretch_factor)**(stretch_index - k_t)) < dz_max):
            self.dz[stretch_index] = dz0 * (1 + stretch_factor)**(stretch_index - k_t)
            stretch_index += 1
...
if __name__ == '__main__':
    ...
    grid2 = Grid_linear_stretched(kmax=176, dz0=10, alpha=0.010)

    grid5 = Grid_three_stage(kmax=128, dz0=10, z_stretch_start=1600, stretch_factor=0.13, dz_max=75)

    # Paper Appendix A example: k_T=120, s=0.0125, dz_start=10, k_M=260, dz_end=185
    grid6 = Grid_stretched_capped(kmax=296, dz_start=5, k_T=210, s=0.05, k_M=256, dz_end=45)  # zsize ~3231 m
