...
INPUT_DIR="/Users/yunpeichu/CLOUDLAB_MIP/Data/CLOUDLAB_MIP_input/ICON_reglatlon"
...
LEVELS_PA="10,50,100,200,300,500,700,1000,2000,3000,5000,7000,10000,12500,15000,17500,20000,22500,25000,30000,35000,40000,45000,50000,55000,60000,65000,70000,75000,77500,80000,82500,85000,87500,90000,92500,95000,97500,100000"
...
find "$INPUT_DIR" -maxdepth 1 -name "*.nc" -print0 | while IFS= read -r -d '' FILE; do
    ...
    OUTPUT_FILE="$OUTPUT_DIR/pl_${BASENAME%.nc}.nc"
    TMP_FILE="$OUTPUT_DIR/tmp_${BASENAME%.nc}.nc"
    ...
    if [ $? -eq 0 ]; then
        echo "  -> Success: Created pl_${BASENAME%.nc}.nc"
    else
        echo "  -> Failed"
    fi
